...
int main(void)
{
	int N, M, **map=NULL, *queue=NULL, front=0, rear=0, count=0;
	bool arrived=false, ***visited=NULL;

	scanf("%d%d", &N, &M);
	map=(int **)malloc(N*sizeof(int *));
	visited=(bool ***)malloc(N*sizeof(bool **));
	queue=(int *)malloc(10*N*M*sizeof(int));
	for(int n=0;n<N;n++)
	{
		...
		{
			scanf("%1d", &map[n][m]);
			visited[n][m]=(bool *)calloc(2,sizeof(bool));
		}
	}
